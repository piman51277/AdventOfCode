...
//print out grid
const minX = dots.reduce((min, [x, y]) => Math.min(min, x), Infinity);
const maxX = dots.reduce((max, [x, y]) => Math.max(max, x), -Infinity);
const minY = dots.reduce((min, [x, y]) => Math.min(min, y), Infinity);
const maxY = dots.reduce((max, [x, y]) => Math.max(max, y), -Infinity);

for(let y = minY; y <= maxY; y++) {
	let line = "";
	for(let x = minX; x <= maxX; x++) {
		if(dots.some(([x2, y2]) => x2 == x && y2 == y)) {
			line += "█";
		} else {
			line += " ";
		}
