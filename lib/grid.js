...
function getNeighborsValue(x, y) {
  var neighbors = [
    ...
    x > 1 && y > 1 ? grid[x - 1][y - 1] : null,
    x > 1 && y < grid[0].length - 1 ? grid[x - 1][y + 1] : null,
    x < grid.length - 1 && y > 1 ? grid[x + 1][y - 1] : null,
...
function getNeighborsValueWithCondition(x, y, condition) {
  var neighbors = [
    //orth
    x > 1 ? [x - 1, y, grid[x - 1][y]] : null,
    x < grid.length - 1 ? [x + 1, y, grid[x + 1][y]] : null,
    y > 1 ? [x, y - 1, grid[x][y - 1]] : null,
    y < grid[0].length - 1 ? [x, y + 1, grid[x][y + 1]] : null,
    //diag
    x > 1 && y > 1 ? [x - 1, y - 1, grid[x - 1][y - 1]] : null,
    x > 1 && y < grid[0].length - 1 ? [x - 1, y + 1, grid[x - 1][y + 1]] : null,
    x < grid.length - 1 && y > 1 ? [x + 1, y - 1, grid[x + 1][y - 1]] : null,
    x < grid.length - 1 && y < grid[0].length - 1 ? [x + 1, y + 1, grid[x + 1][y + 1]] : null
  ];
...
function getNeighborsLocation(x, y) {
  var neighbors = [
    ...
    //diag
    x > 1 && y > 1 ? [x - 1, y - 1] : null,
    x > 1 && y < grid[0].length - 1 ? [x - 1, y + 1] : null,
    x < grid.length - 1 && y > 1 ? [x + 1, y - 1] : null,
...
function getNeighborsLocationWithCondition(x, y, condition) {
  ...
  return neighbors.filter(k => k !== null && condition(k[0], k[1], grid[k[0]][k[1]]));
}
