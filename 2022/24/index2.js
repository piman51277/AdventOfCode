...
function visualize(time) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      const blizzard = nextBlizzardState.find((c) => c[0] === x && c[1] === y);
...
function getTimeTo(start, end, startTime = 0) {
  step();
  let currentGen = [
    {
      position: start,
  ...
  while (true) {
    ...
    if (hasFinished) {
      return hasFinished.time + 1;
    }

    //get next gen's distance from the end
    nextGen = nextGen.map((p) => {
      p.distance = manhattanDistanceTo(p.position, end);
      return p;
    });
...
const time1 = getTimeTo([0, -1], [width - 1, height - 1]);
const time2 = getTimeTo([width - 1, height], [0, 0], time1);
const time3 = getTimeTo([0, -1], [width - 1, height - 1], time2);
console.log(time3);
