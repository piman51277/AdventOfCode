...
//look for all occurances of a
let lowestLength = Infinity;
for (let i = 0; i < cols; i++) {
  for (let j = 0; j < rows; j++) {
    if (inputGrid[j][i] == "a" || inputGrid[j][i] == "S") {
      const path = search(i, j);
      if (path.length < lowestLength && path.length != 0) {
